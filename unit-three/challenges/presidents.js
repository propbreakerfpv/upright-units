...
console.log(presidentsArray.length)



let alivePresidents = presidentsArray.filter((p) => p.passed == undefined);

console.log(alivePresidents);

let deadPresidents = presidentsArray.filter((p) => p.passed !== undefined);

deadPresidents.forEach((p) => {
    console.log(p.first, p.last, "was", p.passed - p.year, "years old when he passed");
})
